...
# --- seam overhang wiring in Construct ---------------------------------------


def _capture_construct_overhang(monkeypatch, rotation):
    """Build a Construct with stubbed rotation/annotation; return the overhang used."""
    import plannotate.annotate as annotate_module
    from plannotate.models import Construct

    captured = {}

    def fake_annotate(seq, db, linear, detailed, cores, overhang_override=None):
        captured["overhang"] = overhang_override
        return pd.DataFrame()

    monkeypatch.setattr(
        _rotate, "rotate_to_origin", lambda *a, **k: rotation, raising=True
    )
    monkeypatch.setattr(annotate_module, "annotate", fake_annotate, raising=True)

    Construct(seq="ACGTACGTACGT", rotate=True)
    return captured["overhang"]


def test_construct_uses_seam_overhang_on_successful_rotation(monkeypatch):
    rotation = _rotate.RotationResult(
        rotated_seq="ACGTACGTACGT",
        offset=3,
        flipped=False,
        ori_name="ori",
        rank=1,
    ...
    assert (
        _capture_construct_overhang(monkeypatch, rotation) == _rotate.ORI_SEAM_OVERHANG
    )


def test_construct_keeps_full_overhang_on_fallback(monkeypatch):
    rotation = _rotate.RotationResult(
        rotated_seq="ACGTACGTACGT",
        offset=2,
        flipped=False,
    ...
    assert _capture_construct_overhang(monkeypatch, rotation) is None
