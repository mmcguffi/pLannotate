...
def _normalize_description_frame(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    to the id, ``type`` to ``misc_feature``, and ``blurb`` to empty. Rows are
    deduplicated on ``sseqid`` (first wins) so the per-hit description merge in
    ``annotate._enrich_hits`` cannot fan a single hit out into several annotations.
    """
    renamed = {
    ...
    # two source columns aliasing to the same canonical name (e.g. both ``id`` and
    # ``accession``) would produce duplicate labels and a later opaque to_sql crash;
    # reject up front with a message that names the offending columns.
    canonical_sources: dict[str, list[str]] = {}
    for source, canonical in renamed.items():
        canonical_sources.setdefault(canonical, []).append(str(source))
    collisions = {
        canonical: sources
    ...
    if collisions:
        detail = "; ".join(
            f"{canonical!r} <- {sources}" for canonical, sources in collisions.items()
        )
        raise ValueError(
            f"descriptions CSV has multiple columns mapping to the same field: {detail}"
        )
    ...
    if "blurb" not in normalized.columns:
        normalized["blurb"] = ""
    normalized = normalized[DESCRIPTION_COLUMNS].copy()
    deduped = normalized.drop_duplicates(subset="sseqid", keep="first")
    if len(deduped) < len(normalized):
        logger.warning(
            "Dropped %d description row(s) with duplicate sseqid",
