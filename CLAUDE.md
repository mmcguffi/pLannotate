...
- **`plannotate/_curation.py`** - Curated selection-marker, origin copy-number, global
  ...
  `data/data/composite_reference_regions.csv`). Tables are keyed on source
  accessions, so they are pinned to one database bundle; run
...
- **`plannotate/_nested.py`** - Conservative nested-feature policy shared by runtime
  ...
  live in `data/data/composite_reference_regions.csv`
