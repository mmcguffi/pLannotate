...
verstrline = open(
    os.path.join(__pkg_name__, "__init__.py"), "r", encoding="utf-8"
).read()
vsre = r"^__version__ = ['\"]([^'\"]*)['\"]"
...
install_requires = []
with open(os.path.join(dir_path, "requirements.txt"), encoding="utf-8") as fh:
    reqs = (
...
setup(
    ...
    entry_points={
        "console_scripts": [
            "plannotate = {}.main:main".format(__pkg_name__),
        ]
    },
)
